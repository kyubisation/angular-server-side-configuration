...
describe('globToRegExp', () => {
  function assertMatch(glob: string, str: string, opts?: Options) {
    expect(globToRegExp(glob, opts).test(str)).toBeTruthy();
  }

  function assertNotMatch(glob: string, str: string, opts?: Options) {
    expect(globToRegExp(glob, opts).test(str)).toBeFalsy();
  }

  function test(globstar: boolean) {
    // Match everything
    assertMatch('*', 'foo');
  ...
  it('should work for all cases', () => {
    ...
    assertNotMatch('http://foo.com/*', 'http://foo.com/bar/baz/jquery.min.js', { globstar: true });

    assertMatch('http://foo.com/*', 'http://foo.com/bar/baz/jquery.min.js', { globstar: false });
    assertMatch('http://foo.com/**', 'http://foo.com/bar/baz/jquery.min.js', { globstar: true });
